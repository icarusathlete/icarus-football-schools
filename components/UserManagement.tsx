...
export const UserManagement: React.FC = () => {
    ...
    const renderAssignmentForm = (
        ...
        <div className="mt-6 p-8 glass-card border-white/10 shadow-2xl relative overflow-hidden bg-brand-900/50 rounded-3xl space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
            
            <div className="space-y-4">
                <label className="block text-[10px] font-black text-brand-500 uppercase tracking-[0.3em] italic">Select Access Trajectory</label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <button 
                        ...
                        className={`p-6 rounded-2xl border text-left transition-all ${
                            form.role === 'player' 
                            ? 'bg-lime text-brand-950 border-lime shadow-[0_0_20px_rgba(195,246,41,0.2)] scale-[1.02]' 
                    ...
                    >
                        ...
                        <h4 className="font-black uppercase tracking-tight mt-4 text-sm">Link to Player</h4>
                    ...
                    <button 
                        ...
                        className={`p-6 rounded-2xl border text-left transition-all ${
                            form.role === 'coach' 
                            ? 'bg-brand-500 text-brand-950 border-brand-500 shadow-[0_0_20px_rgba(14,165,233,0.2)] scale-[1.02]' 
                            : 'bg-white/5 border-white/10 hover:bg-white/10 text-white/50 hover:text-white'
                        }`}
                    >
                        <Shield size={24} className={form.role === 'coach' ? 'text-brand-950' : 'text-white/30'} />
                        <h4 className="font-black uppercase tracking-tight mt-4 text-sm">Setup as Coach</h4>
                    ...
                    >
                        ...
                        <h4 className="font-black uppercase tracking-tight mt-4 text-sm">Academy Admin</h4>
            ...
            <div className="min-h-[100px]">
                {form.role === 'player' && (
                    <div className="space-y-3 animate-in fade-in slide-in-from-left-4 duration-500 bg-white/5 p-6 rounded-2xl border border-white/10">
                        <label className="block text-[10px] font-black text-lime uppercase tracking-[0.3em] italic mb-4">Link Existing Profile</label>
                        <select
                            className="w-full bg-brand-950/80 border border-white/10 rounded-xl p-4 text-white font-bold outline-none focus:ring-2 focus:ring-lime/50 transition-all cursor-pointer uppercase text-xs"
                            value={form.linkedPlayerId || ''}
                            onChange={e => setForm({ ...form, linkedPlayerId: e.target.value })}
                        >
                            <option value="">-- SELECT PLAYER RECORD TO LINK --</option>
                            {players.map(p => (
                                <option key={p.id} value={p.id}>{p.fullName} ({p.memberId})</option>
                            ))}
                        </select>
                        <p className="text-[10px] text-white/30 italic mt-2">The user will only see data (evaluations, attendance) for this specific player.</p>
                    </div>
                )}
                ...
                {form.role === 'coach' && (
                    <div className="space-y-6 bg-white/5 p-6 rounded-2xl border border-white/10 animate-in fade-in slide-in-from-left-4 duration-500">
                        ...
                        <div className="space-y-3">
                            <label className="flex items-center gap-2 text-[10px] font-black text-white/40 uppercase tracking-[0.3em]">
                                ...
                                {venues.map(v => (
                                    <button key={v.id} type="button"
                                        ...
                                        className={`px-4 py-2 rounded-xl text-[10px] font-black border transition-all uppercase tracking-widest ${
                                            ...
                                            ? 'bg-brand-500 text-brand-950 border-brand-500 shadow-[0_0_15px_rgba(14,165,233,0.3)] scale-105'
                ...
                {form.role === 'admin' && (
                    <div className="bg-white/5 border border-white/10 p-6 rounded-2xl flex items-center justify-center animate-in fade-in slide-in-from-left-4 duration-500">
                        <p className="text-[11px] text-white/60 font-medium text-center">
                            <span className="block font-black text-white uppercase tracking-widest mb-1">Administrative Privileges</span>
                ...
                {!form.role && (
                    <div className="h-full flex items-center justify-center border border-dashed border-white/10 rounded-2xl bg-white/[0.02]">
                        <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">Awaiting Trajectory Selection</p>
                    </div>
                )}
            </div>
            ...
            <div className="flex gap-4 pt-4 border-t border-white/10">
                <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-lime text-brand-950 rounded-xl font-black text-[12px] uppercase tracking-widest hover:bg-[#AEEB00] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-xl"
                >
                    {isSaving ? <div className="w-4 h-4 border-2 border-brand-950/30 border-t-brand-950 rounded-full animate-spin" /> : <Shield size={16} />}
                    {saveLabel}
                </button>
                <button onClick={onCancel} className="px-6 py-4 bg-brand-950 text-white/60 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-white/5 transition-all outline-none">
                    Cancel
                </button>
            </div>
        </div>
    ...
    const renderActiveUserRow = (user: User) => (
        <div key={user.id} className="p-8 hover:bg-brand-500/5 transition-all group">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                    <div className="relative">
                        <div className="w-16 h-16 rounded-3xl bg-brand-950 flex items-center justify-center text-xl font-black text-brand-500 overflow-hidden border-2 border-brand-500/20 group-hover:border-brand-500 transition-all shadow-xl">
                            {user.photoUrl ? (
                    ...
                    <div>
                        ...
                        {user.email && <p className="text-[10px] text-white/30 font-bold mt-1">{user.email}</p>}
                        <div className="flex items-center gap-3 mt-2">
                            <div className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] border italic shadow-sm ${
                                ...
                                user.role === 'coach' ? 'bg-brand-500/10 text-brand-500 border-brand-500/20' :
                ...
                <div className="flex items-center gap-3">
                    {editingUserId === user.id ? (
                        <>
                            <button onClick={() => handleSaveEdit(user.id)} className="w-12 h-12 flex items-center justify-center bg-brand-500 text-brand-950 rounded-2xl hover:bg-brand-400 transition-all shadow-xl active:scale-95 border border-brand-500/20" title="Save"><Check size={20} /></button>
                            <button onClick={handleCancelEdit} className="w-12 h-12 flex items-center justify-center bg-brand-900 text-white rounded-2xl hover:bg-brand-950 transition-all shadow-xl active:scale-95 border border-white/5" title="Cancel"><X size={20} /></button>
                        </>
                    ) : (
                        <>
                            <button onClick={() => handleEditClick(user)} className="w-12 h-12 flex items-center justify-center bg-brand-900 text-white rounded-2xl hover:bg-brand-950 hover:text-brand-500 transition-all shadow-xl active:scale-95 border border-white/5" title="Edit"><Edit2 size={20} /></button>
            ...
            {editingUserId === user.id && renderAssignmentForm(editForm, setEditForm, user.id, handleSaveEdit, handleCancelEdit, 'Save Changes', false)}
        </div>
    );
        ...
        <div className="space-y-8 pb-32 animate-in fade-in duration-700 font-display">
            ...
            {pendingUsers.length > 0 && (
                <section className="space-y-4">
                    ...
                    <div className="bg-brand-950 rounded-[2.5rem] border border-yellow-400/10 overflow-hidden shadow-2xl divide-y divide-white/5">
                        {pendingUsers.map(user => (
                            <div key={user.id} className="p-8 hover:bg-yellow-400/5 transition-all">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-center gap-6">
                                        <div className="relative">
                                            <div className="w-16 h-16 rounded-3xl bg-brand-900 flex items-center justify-center text-xl font-black text-yellow-400 overflow-hidden border-2 border-yellow-400/20 shadow-xl">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 mt-2 flex-wrap">
                                                ...
                                                {user.requestedRole && (
                                                    <div className="px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] border bg-white/5 text-white/50 border-white/10">
                                                        Requested: {user.requestedRole}
                                                    </div>
                                                )}
                                                {user.requestReason && (
                                                    <p className="text-[10px] text-white/30 italic font-medium">"{user.requestReason}"</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        {approvingUserId === user.id ? (
                                            <>
                                                <button onClick={() => handleConfirmApprove(user.id)} disabled={isApproving} className="flex items-center gap-2 px-5 py-3 bg-brand-500 text-brand-950 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-400 transition-all disabled:opacity-50 shadow-xl">
                                                ...
                                                <button onClick={() => setApprovingUserId(null)} className="w-10 h-10 flex items-center justify-center bg-white/5 text-white/40 rounded-xl hover:bg-white/10 transition-all"><X size={18} /></button>
                                            </>
                                        ) : (
                                            <>
                                                <button onClick={() => handleApproveClick(user)} className="flex items-center gap-2 px-5 py-3 bg-brand-500/10 text-brand-500 rounded-xl hover:bg-brand-500 hover:text-brand-950 transition-all font-black text-[10px] uppercase tracking-widest border border-brand-500/20 shadow-xl">
                                                    <CheckCircle size={16} /> Approve
                                                </button>
                                                <button onClick={() => handleReject(user.id)} className="flex items-center gap-2 px-5 py-3 bg-red-500/10 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all font-black text-[10px] uppercase tracking-widest border border-red-500/20 shadow-xl">
            ...
            {rejectedUsers.length > 0 && (
                <section className="space-y-4">
                    ...
                    <div className="bg-brand-950/50 rounded-[2.5rem] border border-red-500/10 overflow-hidden divide-y divide-white/5">
                            ...
                            <div key={user.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        {user.email && <p className="text-[10px] text-white/20 font-bold">{user.email}</p>}
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <button onClick={() => handleRestore(user.id)} className="flex items-center gap-2 px-4 py-2.5 bg-white/5 text-white/40 rounded-xl hover:bg-brand-500/10 hover:text-brand-500 transition-all font-black text-[10px] uppercase tracking-widest border border-white/5">
                                        <Clock size={14} /> Move to Pending
                                    </button>
                                    <button onClick={() => handleDeleteClick(user)} className="flex items-center gap-2 px-4 py-2.5 bg-red-500/10 text-red-400/50 rounded-xl hover:bg-red-500 hover:text-white transition-all font-black text-[10px] uppercase tracking-widest border border-red-500/10">
