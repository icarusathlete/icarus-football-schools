                    ...
                    <div className="bg-brand-900 w-full max-w-4xl max-h-[90vh] rounded-[4rem] shadow-3xl border border-white/10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-500">
                        ...
                        <div className="overflow-y-auto px-12 py-12 space-y-12 flex-1 custom-scrollbar">
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 bg-brand-950/30 p-8 rounded-[2.5rem] border border-white/5 shadow-inner max-h-[300px] overflow-y-auto custom-scrollbar">
                                    {players.map(p => (
                                        <div key={p.id} onClick={() => toggleStarter(p.id)} className={`cursor-pointer p-4 rounded-2xl border transition-all flex items-center gap-4 ${starters.has(p.id) ? 'bg-brand-500/10 border-brand-500 shadow-xl' : 'bg-brand-900 border-white/5'}`}>
                                            <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${starters.has(p.id) ? 'bg-brand-500 border-brand-500' : 'border-brand-800'}`}>{starters.has(p.id) && <Check size={14} className="text-brand-950" strokeWidth={4} />}</div>
                                            <span className={`text-xs font-black uppercase italic tracking-tight truncate ${starters.has(p.id) ? 'text-brand-500' : 'text-brand-700'}`}>{p.fullName}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
                        </div>
                        <div className="px-12 py-10 border-t border-white/5 bg-brand-950/50 flex gap-6"><button onClick={() => setShowForm(false)} className="px-10 py-5 text-brand-700 font-black hover:text-white transition-all text-xs uppercase tracking-[0.3em] italic">Abort Engagement</button><button onClick={handleSaveMatch} className={`flex-1 py-5 font-black rounded-3xl shadow-3xl transition-all flex items-center justify-center gap-4 text-xs uppercase tracking-[0.4em] italic ${newMatch.isLive ? 'bg-red-600 text-white shadow-red-600/20' : 'bg-brand-500 text-brand-950 shadow-brand-500/20'}`}><Save size={20} /> {newMatch.isLive ? 'DEEP SCAN DEPLOYMENT' : 'STORE TACTICAL DATA'}</button></div>
                    </div>
                </div>
            )}
