...
export const MessagingManager: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-white space-y-8 pb-32 animate-in fade-in duration-700">
            ...
            <div className="bg-brand-secondary p-10 md:p-14 rounded-[3rem] border-b-4 border-brand-primary shadow-2xl relative overflow-hidden group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-10">
                    ...
                    <button 
                        ...
                        className="bg-brand-primary text-brand-secondary px-12 py-6 rounded-3xl shadow-[0_20px_40px_rgba(0,200,255,0.2)] hover:scale-105 active:scale-95 transition-all font-black text-sm uppercase tracking-widest italic border-b-4 border-brand-secondary/20 flex items-center gap-4 group/btn"
                    >
            ...
            <div className="max-w-[1600px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className="lg:col-span-3 space-y-8">
                    <div className="surface-card p-8 rounded-[2.5rem] border border-slate-200 shadow-xl relative overflow-hidden">
                        ...
                        <div className="space-y-6">
                            {[
                                ...
                                { label: 'Queue Depth', value: '0', icon: Clock, color: 'text-slate-300' },
                                { label: 'System Health', value: '100%', icon: Zap, color: 'text-green-500' }
                            ].map((stat, i) => (
                                <div key={i} className="group p-5 rounded-2xl bg-slate-50 border border-slate-100 hover:border-brand-primary/20 transition-all">
                                    <div className="flex items-center justify-between">
                                        <div className="flex flex-col gap-1">
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{stat.label}</span>
                                            <span className="text-3xl font-black text-slate-900 italic tracking-tighter group-hover:text-brand-primary transition-colors">{stat.value}</span>
                                        </div>
                    ...
                    <div className="surface-card p-8 rounded-[2.5rem] border border-slate-200 shadow-xl">
                        ...
                        <div className="space-y-2">
                             {[
                                ...
                                { label: 'SMS Stream', icon: MessageSquare }
                                 ...
                                 <button key={i} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${i === 0 ? 'bg-brand-primary text-brand-secondary border-brand-primary shadow-lg shadow-brand-primary/20' : 'text-slate-400 hover:bg-slate-50 border-transparent hover:border-slate-100'}`}>
                                     <f.icon size={14} />
                                     {f.label}
                                 </button>
                             ))}
                ...
                <div className="lg:col-span-9 space-y-8">
                    <div className="flex items-center justify-between mb-2">
                        <div className="space-y-1">
                            <h3 className="text-2xl font-black text-slate-900 italic uppercase tracking-tighter">Transmission <span className="text-brand-primary">Logs</span></h3>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="px-4 py-2 bg-slate-50 rounded-full border border-slate-200 flex items-center gap-3">
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        {messages.length === 0 ? (
                            <div className="py-32 text-center bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-100 flex flex-col items-center">
                                <Search size={64} className="text-slate-200 mb-6" />
                                <p className="text-[12px] font-black text-slate-300 uppercase tracking-widest italic">No Transmission Logs Found</p>
                            </div>
                        ) : (
            ...
            {showCompose && (
                <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white rounded-t-[3rem] sm:rounded-[3rem] shadow-3xl w-full max-w-4xl overflow-hidden animate-in slide-in-from-bottom sm:zoom-in-95 duration-500 border border-slate-200 flex flex-col max-h-[95vh] sm:max-h-[85vh]">
                        {/* Modal Header */}
                        <div className="px-12 py-10 bg-slate-50 flex justify-between items-center relative overflow-hidden border-b border-slate-100">
                            ...
                            <div className="space-y-2 relative z-10">
                                <h3 className="font-black text-3xl text-slate-900 italic uppercase tracking-tighter">Compose <span className="text-brand-primary">Broadcast</span></h3>
                                <p className="text-[10px] text-brand-primary font-black uppercase tracking-[0.3em] italic">Deploy High-Impact Communication</p>
                            </div>
                            <button onClick={() => !isSending && setShowCompose(false)} className="p-4 hover:bg-slate-100 rounded-3xl text-slate-300 transition-all"><X size={24} /></button>
                        </div>

                        {/* Quick Templates Bar */}
                        <div className="px-12 py-6 bg-slate-50/50 border-b border-slate-100 flex items-center gap-4 overflow-x-auto no-scrollbar">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic shrink-0">Quick Templates:</span>
                            <div className="flex gap-2">
                                <button type="button" onClick={() => applyTemplate('rainout')} className="px-5 py-2.5 bg-white border border-slate-200 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-500 hover:border-brand-primary transition-all shadow-sm">⚽ Rainout</button>
                                <button type="button" onClick={() => applyTemplate('schedule')} className="px-5 py-2.5 bg-white border border-slate-200 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-500 hover:border-brand-primary transition-all shadow-sm">📅 Schedule</button>
                                <button type="button" onClick={() => applyTemplate('fees')} className="px-5 py-2.5 bg-white border border-slate-200 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-500 hover:border-brand-primary transition-all shadow-sm">💳 Fee Reminder</button>
                            </div>
                        </div>

                        <form onSubmit={handleSend} className="p-12 space-y-10 flex-1 overflow-y-auto">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Target Audience</label>
                                    <div className="relative group">
                                        <select 
                                            className="w-full p-5 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary font-black text-slate-900 text-xs uppercase italic tracking-widest appearance-none transition-all"
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Delivery Channel</label>
                                            ...
                                            <button
                                                ...
                                                className={`py-5 rounded-2xl border flex flex-col items-center justify-center gap-2 transition-all ${
                                                    newMessage.type === chan.id 
                                                        ? 'bg-brand-primary border-brand-primary text-brand-secondary shadow-xl shadow-brand-primary/20' 
                            ...
                            {newMessage.targetAudience !== 'all' && (
                                <div className="space-y-3 animate-in slide-in-from-top duration-500">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Identify Selection</label>
                                    <select 
                                        className="w-full p-5 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary font-black text-slate-900 text-xs uppercase italic tracking-widest appearance-none transition-all"
                            ...
                            <div className="space-y-3">
                                ...
                                <input 
                                    className="w-full p-6 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary font-black text-slate-900 transition-all placeholder:text-slate-300 uppercase italic text-sm"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Log Content</label>
                                <textarea 
                                    className="w-full p-6 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary font-medium text-slate-900 shadow-sm transition-all h-52 placeholder:text-slate-300 text-sm leading-relaxed italic"
                            ...
                            <div className="pt-10 flex flex-col gap-6 border-t border-slate-100">
                                ...
                                <div className="flex gap-4">
                                    <button 
                                        ...
                                        className="flex-1 py-5 text-slate-400 font-black hover:text-slate-600 rounded-3xl transition-all text-xs uppercase tracking-widest italic border border-transparent hover:border-slate-100"
                                    >
                                        Discard_Draft
                                    </button>
                                    <button 
                                        ...
                                        className={`flex-[2] py-5 font-black rounded-3xl shadow-2xl transition-all text-xs uppercase tracking-[0.2em] italic flex items-center justify-center gap-4 ${
                                            isSending ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-brand-primary text-brand-secondary hover:scale-[1.02] active:scale-95 shadow-brand-primary/20'
                                        }`}
                                    >
                                        {isSending ? (
